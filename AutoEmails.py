...
if __name__ == '__main__':
	...
	SEED_data_issues = SEED_data_issues.drop(SEED_data_issues[SEED_data_issues['Compliant'] == 'Applied'].index)
	SEED_data_issues.to_excel('Output\\SEED_data_issues'+tstamp+'.xlsx')
	SEED_data_noissues = SEED_data.loc[No_Issues_filter_all]
	SEED_data_noissues.to_excel('Output\\SEED_data_noissues'+tstamp+'.xlsx')
	...
	SEED_data_manual_compliance = SEED_data.drop(SEED_data[SEED_data['Compliant'] != 'Applied'].index)
	SEED_data_manual_compliance.to_excel('Output\\SEED_data_manual_compliance'+tstamp+'.xlsx')
	...
	for index, row in SEED_data_issues_resub.iterrows():
		...
		if is_valid != False:
			...
			pass
	"""
	for index, row in SEED_data_issues.iterrows():
		is_valid = validate_email(row['Property Data Administrator - Email'], verify = True)
		if is_valid != False:
			...
			try:
				...
				messageBody = messageBody.replace('[ELECPERCENT]','0')
			messageBody = messageBody.replace('[ENERGYSTARScore]',str(row['ENERGY STAR Score']))
			#messageBody = messageBody.replace('[DUEDATE]','by April 16th, 2021')			
			messageBody = messageBody.replace('[DUEDATE]','as soon as possible')														
	...
	"""
	for index, row in SEED_data_noissues.iterrows():
	...
	for index, row in SEED_data_manual_compliance.iterrows():
		data_View_2 = data_View.drop(data_View[data_View['PM Property ID'] != row['PM Property ID']].index)
		data_View_2 = data_View_2.set_index('PM Property ID')
		property_name = row['Property Name']
		property_name = property_name.replace(' | ', " ")
